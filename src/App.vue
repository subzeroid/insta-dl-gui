...
<template>
  ...
  <div v-else class="flex h-full flex-col">
    <header v-if="showChrome" class="flex items-center justify-between border-b border-line bg-surface-1 px-4 py-3">
      ...
      <nav class="flex items-center gap-1">
        <RouterLink
          ...
          :key="r.path"
          :to="r.path"
          class="rounded-lg px-3 py-1.5 text-sm text-slate-400 hover:bg-surface-3 hover:text-slate-200"
          active-class="!text-slate-100 bg-surface-3"
        >
          {{ r.label }}
        </RouterLink>
        <button
          class="ml-3 rounded-full border border-line px-3 py-1 text-xs tabular-nums text-slate-300 hover:bg-surface-3"
